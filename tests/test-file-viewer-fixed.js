...
`;
    ...
    // Create test markdown file using API
    const mdResponse = await page.evaluate(async (content) => {
    ...
    console.log('MD file creation response:', mdResponse);

    // Create test text file using API
    const txtResponse = await page.evaluate(async (content) => {
      const token = localStorage.getItem('adminToken');
    ...
    console.log('TXT file creation response:', txtResponse);
    ...
    await expect(page.locator('text=test-viewer.md')).toBeVisible({ timeout: 5000 });
    await expect(page.locator('text=test-viewer.txt')).toBeVisible({ timeout: 5000 });
  ...
  test('should clean up test files', async () => {
    ...
    for (const fileName of testFiles) {
      try {
        ...
        if (!result.success) {
          console.warn(`Could not delete ${fileName}: ${result.message}`);
        }
      } catch (error) {
