...
pub async fn get_github_projects(
    ...
    let config = match load_github_config() {
        Ok(config) => config,
        Err(e) => {
            return Ok(HttpResponse::InternalServerError().json(ApiResponse::<()>::error(&format!("Error loading GitHub config: {}", e))));
        }
    };
    ...
    for repo in config.repositories {
        ...
        match fetch_github_project(&repo).await {
            ...
            Err(e) => {
                eprintln!("Error fetching GitHub project {}/{}: {}", repo.owner, repo.repo, e);
...
pub async fn refresh_github_cache(
    ...
    for repo in config.repositories {
        ...
        match fetch_github_project(&repo).await {
            ...
            Err(e) => {
                eprintln!("Error refreshing GitHub project {}/{}: {}", repo.owner, repo.repo, e);
            }
        }
    }
    
    Ok(HttpResponse::Ok().json(ApiResponse::success(format!("Refreshed {} projects", refreshed_count))))
}

async fn fetch_github_project(repo: &GitHubRepo) -> Result<GitHubProject, anyhow::Error> {
    let readme_html = match fetch_github_readme(&repo.owner, &repo.repo).await {
        Ok(markdown) => {
        ...
        Err(_) => "README not available".to_string(),
    };
    
    let (stars, forks, description) = fetch_github_repo_info(&repo.owner, &repo.repo).await?;
